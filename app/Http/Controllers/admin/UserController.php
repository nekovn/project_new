...
use App\Models\UserModel as MainModel; // đặt tên lại là SliderModel
...
class UserController extends Controller
{
    ...
    public function changePassword(MainRequest $request){
        if($request->method ()=='POST'){ //nếu method là post
            ...
            return redirect ()->route ($this->controllerName)->with ('zvn_notify',"Thay đổi password thành công !");
        }
    ...
    public function changeLevel(MainRequest $request){
        if($request->method ()=='POST'){ //nếu method là post
            $params = $request->all(); // lấy tất cả param dc post qua
            $this->model->saveItems ($params,['task'=>'change-level-post']);
