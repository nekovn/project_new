$(document).ready(function() {
	...
	let $selectChangeAttr = $("select[name =  select_change_attr]"); // thay đổi khi select phần tử
	...
	$("a.select-field").click(function(e) {
		...
		$("button.btn-active-field").html(fieldName + ' <span class="caret"></span>');//cho noi dung vao class do
    	$inputSearchField.val(field);
	});

	$btnSearch.click(function() {

		var pathname	= window.location.pathname;//lay duong dẫn hiện tại
		let searchParams= new URLSearchParams(window.location.search);// lấy tên param trên url
		params 			= ['page', 'filter_status', 'select_field', 'select_value'];

		let link		= "";
		$.each( params, function( key, value ) {
			if (searchParams.has(value) ) {
                //searchParams.get(value) : lấy giá trị của tên params
				link += value + "=" + searchParams.get(value) + "&"// filter_status = active
			}
		});

		let search_field = $inputSearchField.val();
		let search_value = $inputSearchValue.val();
		//search_value.replace(/\s+/g, '+') :: lat tat cả giá chỉ trong value
...
//slice(0,-1) :: xóa dau & o cuối cùng , từ o -> -1 : 0 là f , -1 là e
	...
	$selectChangeAttr.on('change', function() {
		let select_value = $(this).val();
		let $url = $(this).data('url');//lấy url của data_url
		window.location.href = $url.replace('value_new', select_value); // replace trên url thay đổi value_new thành select_value
	});
